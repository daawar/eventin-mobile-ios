...
//http://api.eventful.com/json/categories/list?app_key=2Dmmk5bhzHWSLJzj
...
#define EVENTS_URL @"events/search?app_key=%@&category=%@&location=%@&date=Future&sort_order=date&page_number=%d&page_size=%d"
...
-(void)fetchEventsForCategoryName:(NSString*)name withPageNumber:(NSUInteger)pageNumber andPageSize:(NSUInteger)pageSize andCity:(NSString*) city onSuccess:(void (^)(NSArray *events,NSUInteger totalEventsCount))success
    ...
    NSString *url = [NSString stringWithFormat:EVENTS_URL,APP_KEY,[[[name lowercaseString] encodeAndToSymbol] encodeEmptySpaces], city,pageNumber,pageSize];
